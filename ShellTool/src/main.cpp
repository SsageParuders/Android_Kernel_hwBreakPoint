...
int main(int argc, char *argv[]) {
	...
	bool is_printf_more = false;

	while ((opt = getopt(argc, argv, "p:a:l:t:d:")) != -1) {
		switch (opt) {
		case 'p':
		...
		case 'd':
			if (strcmp(optarg, "T") == 0) {
				...
				is_printf_more = false;
			}
			break;
		default:
			printf("Usage: %s [-p <attach_pid>] [-a <memory_hex_addr>] [-l <hw_breakpoint_len>] [-t <hw_breakpoint_type>] [-d <is printf more?>] arg1 ...\n", argv[0]);
			printf("Example: %s -p 8072 -a 9ECF6140 -l 8 -t rw -d F\n", argv[0]);
			return 0;
			break;
		}
	}
	...
	if(is_printf_more){ // 开启了debug模式才打印
		printf( 
			"======================================================\n"
			"本驱动名称: Linux ARM64 硬件断点进程调试驱动\n"
			"本驱动接口列表：\n"
	...
	for (uint64_t hwBpHandle : vHwBpHandle) {
		...
		if(is_printf_more || b == 1){ // 开启了debug模式 或者 b值为1
			printf("==========================================================================\n");
